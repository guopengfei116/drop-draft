(function (window) {
    ...
    // 由于会经常调用itcast.prototype,因此为其添加一个简写方式
    itcast.fn = itcast.prototype = {
        constructor: itcast,
        selector: '', // 用来判断是itcast对象的属性
        ...
        init: function (selector, context) {
            ...
            if (itcast.isArrayLike(selector)) {
                // 将dom数组中每一个dom元素添加到当前itcast实例上，并且长度自动更新
    ...
    itcast.extend({
        ...
        hasClass: function (dom, className) {
            return (' ' + dom.className + ' ')
                    .indexOf(' ' + itcast.trim(className) + ' ') > -1;
        },
    ...
    itcast.fn.extend({
        css: function (name, value) {
            ...
            if (value === undefined) {
                ...
                if (typeof name === 'object') {
                    ...
                    // 默认获取itcast对象上的一个dom元素的样式值
                    // 首先判断当前itcast对象上是否有dom元素，
                    // 如果有，就返回第一个dom元素的样式值
                    // 否则就是没有，那么就返回undefined。
                    return this.length > 0 ?
                        itcast.getCss(this[0], name) :
                        undefined;
                }
            } else { // 如果value有值，那么就是itcast对象上的所有dom元素设置单个样式值
                return this.each(function () {
        ...
        hasClass: function (className) {
            // 默认还是判断itcast对象上的第一个dom元素是否就有指定的样式类
            return this.length > 0 ?
                itcast.hasClass(this[0], className) :
                false;
        },
        addClass: function (className) {
            // 给itcast对象上所有dom元素都添加指定的样式类
            return this.each(function () {
                // this -> 遍历到的每一个itcast对象上的dom元素
        ...
        toggleClass: function (className) {
            return this.each(function () {
                ...
                // if (itcast.hasClass(this, className)) {
                //     itcast.removeClass(this, className);
                // } else {
                //     itcast.addClass(this, className);
                // }
    ...
    itcast.fn.extend({
        attr: function (name, value) {
            ...
            // 默认返回itcast对象上的第一dom元素的指定属性值。
            if (value === undefined) {
                return this.length > 0 ? itcast.getAttr(this[0], name) :
                    undefined;
            } else { // 如果不为undefined，表示给itcast对象上的所有dom元素设置指定的属性值
                return this.each(function () {
        ...
        html: function (html) {
            // 如果html没有传入值的话，就表示获取itcast对象上第一个dom元素的innerHTML属性值
            if (html === undefined) {
                return this.length > 0 ? itcast.getHTML(this[0]) : undefined;
            } else { // 如果被传入值，那么就是给itcast对象上的所有dom元素设置innerHTML属性
                return this.each(function () {
    ...
    itcast.extend({
        ...
        getTweens: function (time, locations, target, dur, easingName) {
            var k, o = {};
            for (k in target) {
                o[k] = itcast.easing[easingName](null, time, locations[k],
        ...
        animate: function (target, dur, easingName) {
            return this.each(function (dom) {
                var locations = itcast.getLocations(dom, target),
                    distances = itcast.getDistances(locations, target),
                    startTime,
                    timer = dom.timerId;
                startTime = +new Date;
                // 把时间转换成毫秒值，动画开始时间。
    ...
    itcast.extend({
        ...
        Ajax: function (options) {
            ...
            // 将默认Ajax设置拷贝到context
            itcast.extend(itcast.AjaxSetting, context);
            // 将用户自定义设置覆盖context
            itcast.extend(options, context);
            if (context.datatype === 'jsonp') {
                if (!options.callback) {
