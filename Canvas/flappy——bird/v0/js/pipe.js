...
function Pipe(x) {
    ...
    this.x = x;
    this.width = Pipe.IMG_WIDTH; // 管道宽度

    // 上管道高度为80 到 220之间
    this.height = Math.random() * 200 + 80;  // 在类中的方法或者实例方法中计算管道高度
...
// 添加初始化函数
Pipe.init = function (ctx, pipeDown, pipeUp) {
    Pipe.ctx = ctx;
...
extend(Pipe.prototype, {
    ...
    // 绘制
    draw: function () {
        Pipe.ctx.drawImage(Pipe.pipeDown, this.x, this.downY);
        Pipe.ctx.drawImage(Pipe.pipeUp, this.x, this.upY);
        this.drawPipePath();
    },
    ...
    drawPipePath: function () {
        ...
        Pipe.ctx.rect(this.x, this.downY, Pipe.IMG_WIDTH, Pipe.IMG_HEIGHT);
        Pipe.ctx.rect(this.x, this.upY, Pipe.IMG_WIDTH, Pipe.IMG_HEIGHT);
        Pipe.ctx.stroke();
    },
