(function (w) {
    ...
    function Timer( options ) {
        ...
        this.durationTime = 0;  // 记录总时间(游戏运行时长)
    ...
    util.extend( Timer.prototype, {
        ...
        _update: function( delay ) {
            ...
            var hours = Math.floor(this.durationTime / (1000 * 60 * 60));
            var minutes = Math.floor(this.durationTime % (1000 * 60 * 60) / (1000 * 60));
            var seconds = Math.floor(this.durationTime % (1000 * 60) / 1000);
            this.text = '坚持了' + hours + '小时' + minutes + '分钟' + seconds + '秒！';
        }
    });

    // 暴漏到全局
    w.getTimer = function ( options ) {
